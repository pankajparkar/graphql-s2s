...
var _defineProperties = __webpack_require__(64);
...
var _require = __webpack_require__(33),
    printError = _require.printError;
...
module.exports.GraphQLError = __webpack_require__(17).GraphQLError;
module.exports.syntaxError = __webpack_require__(57).syntaxError;
module.exports.locatedError = __webpack_require__(56).locatedError;
module.exports.printError = __webpack_require__(33).printError;
module.exports.formatError = __webpack_require__(55).formatError;
...
var _require = __webpack_require__(35),
    getLocation = _require.getLocation;
...
module.exports.invariant = function (condition, message) {
...
var _start = void 0;
var startTime = function startTime(anything) {
    _start = Date.now();
    return anything;
};
var logTime = function logTime(anything, label) {
    if (!_start) _start = Date.now();
...
var escapeGraphQlSchema = function escapeGraphQlSchema(sch) {
    var cr = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '_cr_';
 ...
 */
var GRAPHQLSCALARTYPES = { 'ID': true, 'String': true, 'Float': true, 'Int': true, 'Boolean': true };
 ...
 */
var removeAlias = function removeAlias() {
 ...
 */
var addMetadataToProperty = function addMetadataToProperty(queryProp, parentTypeAST, schemaAST) {
        ...
        if (schemaProp) return {
            ...
            properties: queryProp.properties && queryProp.properties.length > 0 ? chain(schemaProp.details.result.name).next(function (typename) {
                ...
                return parentTypeAST ? queryProp.properties.map(function (queryProp) {
                    return addMetadataToProperty(queryProp, parentTypeAST, schemaAST);
 ...
 */
var addMetadataToAST = function addMetadataToAST(operation, schemaAST) {
    var queryType = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'Query';
    return chain(
    ...
    chain(schemaAST['get' + queryType] = schemaAST.find(function (x) {
        return x.type == 'TYPE' && x.name == queryType;
    })).next(function () {
        return schemaAST['get' + queryType];
    }).val()).next(function (parentTypeAST) {
        return chain(operation && operation.properties ? operation.properties.map(function (prop) {
            return addMetadataToProperty(prop, parentTypeAST, schemaAST);
        }) : []).next(function (body) {
...
var parseProperties = function parseProperties(selectionSet) {
    return !selectionSet ? null : (selectionSet.selections || []).map(function (x) {
        return {
...
var parseKeyValue = function parseKeyValue(_ref2) {
    var kind = _ref2.kind,
        name = _ref2.name,
        value = _ref2.value;
    ...
    return {
        ...
        value: !name && !value.kind ? { kind: kind, value: value } : {
            kind: value.kind,
            value: value.name ? value.name.value : value.fields ? value.fields.map(function (f) {
                return parseKeyValue(f);
            }) : value.values ? value.values.map(function (v) {
                return v.value ? parseKeyValue(v) : v.fields.map(function (f) {
                    return parseKeyValue(f);
                });
            }) : value.value
        }
...
var parseArguments = function parseArguments(astArgs) {
    return !astArgs || !astArgs.length ? null : astArgs.map(function (a) {
        return parseKeyValue(a);
    });
};
...
var _graphQlQueryTypes = { 'query': 'Query', 'mutation': 'Mutation', 'subscription': 'Subscription'
     ...
     */
};var getQueryAST = function getQueryAST(query, operationName, schemaAST) {
    var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};

    var parsedQuery = (parse(query) || {}).definitions || [];
    var ast = parsedQuery.find(function (x) {
        return x.kind == 'OperationDefinition' && (!operationName || x.name.value == operationName);
    });
    if (!ast) {
        if (operationName) throw new Error('Invalid Graphql query. Operation name \'' + operationName + '\' is not defined in the query.');else throw new Error('Invalid Graphql query. No \'OperationDefinition\' defined in the query.');
    }
    ...
    if (ast) {
        ...
        var postProcess = options.defrag ? function (o) {
            return addMetadataToAST(defrag(o), schemaAST, _graphQlQueryTypes[ast.operation]);
        } : function (o) {
            return o;
        };
        var output = postProcess(addMetadataToAST(operation, schemaAST, _graphQlQueryTypes[ast.operation]));
        (0, _assign2.default)(output, {
            filter: function filter(fn) {
                return filterQueryAST(output, fn);
            },
            some: function some(fn) {
                return detectQueryAST(output, fn);
            },
            propertyPaths: function propertyPaths(fn) {
                return getQueryASTPropertyPaths(output, fn);
            }
        });
        return output;
    } else return null;
...
var stringifyOperation = function stringifyOperation() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    var acc = [];
    acc.push(operation.type || 'query');
    if (operation.name) acc.push(operation.name);
    if (operation.variables && operation.variables.length > 0) acc.push('(' + operation.variables.map(function (v) {
...
var filterQueryAST = function filterQueryAST() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var predicate = arguments[1];
    var onlyReturnBody = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

    if (operation.properties && predicate) {
        var filteredBody = operation.properties.filter(function (x) {
            return predicate(x);
        }).map(function (x) {
            return x.properties && x.properties.length > 0 ? (0, _assign2.default)({}, x, { properties: filterQueryAST(x, predicate, true) }) : x;
        });

        return onlyReturnBody ? filteredBody : (0, _assign2.default)({}, operation, { properties: filteredBody });
    } else return onlyReturnBody ? null : operation;
};

var detectQueryAST = function detectQueryAST() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var predicate = arguments[1];
    return operation.properties && predicate && (operation.properties.some(function (x) {
        return predicate(x);
    }) || operation.properties.some(function (x) {
        return detectQueryAST(x, predicate);
    }));
};

var getQueryASTPropertyPaths = function getQueryASTPropertyPaths() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var predicate = arguments[1];
    var parent = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';

    var prefix = parent ? parent + '.' : parent;
    if (operation.properties && predicate) return operation.properties.reduce(function (acc, p) {
        if (predicate(p)) acc.push({ property: prefix + p.name, type: p.type });
        if (p.properties) acc.push.apply(acc, (0, _toConsumableArray3.default)(getQueryASTPropertyPaths(p, predicate, prefix + p.name)));
        return acc;
    }, []);else return [];
};
...
var buildQuery = function buildQuery() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var skipOperationParsing = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    return chain((operation.properties || []).map(function (a) {
...
var stringifyFragments = function stringifyFragments() {
    ...
    return fragments.map(function (f) {
        return 'fragment ' + f.name + ' on ' + f.type + ' ' + buildQuery(f, true);
    }).join('\n');
};
...
var buildSingleQuery = function buildSingleQuery(AST) {
    if (AST && AST.name) {
        ...
        var fields = AST.properties && AST.properties.length > 0 ? buildQuery(AST, true) : '';
        return AST.kind == 'FragmentSpread' ? '...' + fnName : '' + fnName + (args ? '(' + args + ')' : '') + fields;
    } else return '';
};

var stringifyValue = function stringifyValue(_ref4) {
    var kind = _ref4.kind,
        value = _ref4.value;

    if (Array.isArray(value)) return kind == 'ListValue' ? '[' + stringifyArgs(value) + ']' : '{' + stringifyArgs(value) + '}';else return kind == 'Variable' ? '$' + value : kind == 'StringValue' ? '"' + value + '"' : value;
};

var stringifyArgs = function stringifyArgs() {
    var args = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    return '' + args.map(function (arg) {
        return Array.isArray(arg) ? '{' + stringifyArgs(arg) + '}' : '' + (arg.name ? arg.name + ':' : '') + stringifyValue(arg.value);
    }).join(',');
};
...
var defrag = function defrag(operation) {
    if (operation && operation.fragments && operation.fragments.length > 0) {
        // reset cache
        _defragCache = {};
        var properties = replaceFragmentsInProperties(operation.properties, operation.fragments);
        // reset cache
        _defragCache = {};
        return (0, _assign2.default)({}, operation, { properties: properties, fragments: null });
 ...
 */
var genericDefaultNameAlias = function genericDefaultNameAlias(genName) {
	if (!genName) return '';
...
var getAliasName = function getAliasName(genericType, metadata) {
	return memoizedGenericNameAliases[genericType] || chain(genericType.match(/.*</)[0]).next(function (genericStart) {
		return getAllAliases(metadata).find(function (x) {
			return x.schemaName.indexOf(genericStart) == 0;
		});
	}).next(function (aliasObj) {
		var alias = aliasObj && aliasObj.body ? getGenericAlias(aliasObj.body)(genericType) : genericDefaultNameAlias(genericType);
...
var getAllAliases = function getAllAliases(metadata) {
 ...
 */
var getTypeDetails = function getTypeDetails(t, metadata, genericParentTypes) {
...
var getPropertyValue = function getPropertyValue(_ref3, mapResultName) {
	var name = _ref3.name,
	    params = _ref3.params,
	    result = _ref3.result;
		...
		return _(blockParts).reduce(function (a, part) {
			...
			if (p.indexOf('#') == 0) a.comments.push(p);else {
				...
				var propDetails = paramsMatch ? chain(prop.split(paramsMatch[0])).next(function (parts) {
					return { name: parts[0].trim(), metadata: mData, params: paramsMatch[1], result: getTypeDetails((parts[1] || '').replace(':', '').trim(), metadata, baseObj.genericTypes) };
				}).val() : chain(prop.split(':')).next(function (parts) {
					return { name: parts[0].trim(), metadata: mData, params: null, result: getTypeDetails((parts[1] || '').trim(), metadata, baseObj.genericTypes) };
				}).val();
				a.props.push({
...
var getSchemaObject = function getSchemaObject(definitions, typeName, nameRegEx, metadata) {
	return _.toArray(_(definitions).filter(function (d) {
		...
		if (typeName == 'scalar') return {
			...
			var baseObj = { type: objectType, name: name, genericTypes: genericTypes };
...
var getGenericAlias = function getGenericAlias(s) {
	return !s ? genericDefaultNameAlias : function (genName) {
...
var getObjWithExtensions = function getObjWithExtensions(obj, schemaObjects) {
	if (obj && schemaObjects && obj.inherits) {
		...
		var superClassesWithInheritance = superClass.map(function (subClass) {
		...
		var objWithInheritance = {
			...
			blockProps: superClassesWithInheritance instanceof Array ? _.toArray(_.flatten(_.concat(_.flatten(superClassesWithInheritance.map(function (subClass) {
				return subClass.blockProps;
			})), obj.blockProps))) : _.toArray(_.flatten(_.concat(superClassesWithInheritance.blockProps, obj.blockProps)))
		};
...
var createNewSchemaObjectFromGeneric = function createNewSchemaObjectFromGeneric(_ref4, schemaBreakDown, memoizedNewSchemaObjectFromGeneric) {
	var originName = _ref4.originName,
	    isGen = _ref4.isGen,
	    name = _ref4.name;

	if (!memoizedNewSchemaObjectFromGeneric) throw new Error('Missing required argument. \'memoizedNewSchemaObjectFromGeneric\' is required.');
	if (isGen && memoizedNewSchemaObjectFromGeneric[name]) return memoizedNewSchemaObjectFromGeneric[name];else if (isGen) {
		...
		var blockProps = baseGenObj.blockProps.map(function (prop) {
			...
			if (isTypeGeneric(prop.details.result.name, baseGenObj.genericType)) {
				...
				if (prop.details.result.dependsOnParent) {
					...
					// e.g. PagedProduct
					var concreteGenPropName = createNewSchemaObjectFromGeneric(concreteGenProp, schemaBreakDown, memoizedNewSchemaObjectFromGeneric).obj.name;
					// e.g. [PagedProduct]
					var originalConcretePropTypeName = propTypeIsArray ? '[' + concreteGenPropName + ']' : concreteGenPropName;
...
var buildSchemaString = function buildSchemaString(schemaObjs) {
	var part_01 = _(schemaObjs).filter(function (x) {
		return !x.genericType && x.type != 'ABSTRACT';
	}).map(function (obj) {
		return parseSchemaObjToString(obj.comments, obj.type, obj.name, obj.implements, obj.blockProps, obj.extend);
	}).join('\n');

	var resolvedGenericTypes = {};
	_(memoizedGenericSchemaObjects).filter(function (x) {
		return !x.dependsOnParent;
	}).forEach(function (value) {
		return createNewSchemaObjectFromGeneric(value, schemaObjs, resolvedGenericTypes).stringObj;
	});

	var part_02 = _(resolvedGenericTypes).map(function (x) {
		return x.stringObj;
	}).join('\n');
	...
	return chain(getSchemaBits(graphQlSchema)).next(function (schemaBits) {
		...
		if (includeNewGenTypes) {
			...
			_(memoizedGenericSchemaObjects).filter(function (x) {
				...
				return createNewSchemaObjectFromGeneric(value, v, resolvedGenericTypes);
			});

			return v.concat(_.toArray(_(resolvedGenericTypes).map(function (x) {
				return x.obj;
			})));
		} else return v;
	}).val();
...
var graphqls2s = {
	getSchemaAST: function getSchemaAST(graphQlSchema) {
		return chain(resetMemory()).next(function () {
			...
			return getSchemaParts(data.stdSchema, data.metadata, true);
		}).next(function (v) {
	...
	transpileSchema: function transpileSchema(graphQlSchema) {
		return chain(resetMemory()).next(function () {
			...
			return buildSchemaString(getSchemaParts(data.stdSchema, data.metadata));
		}).next(function (v) {
			resetMemory();return v;
		}).val();
	},
	extractGraphMetadata: extractGraphMetadata,
	getGenericAlias: getGenericAlias,
	getQueryAST: getQueryAST,
	buildQuery: buildQuery,
	isTypeGeneric: isTypeGeneric
};
...
var extractGraphMetadata = function extractGraphMetadata() {
	...
	var graphQlMetadata = chain(_(attrMatches).map(function (m) {
		return chain(m.split(carrReturnEsc)).next(function (parts) {
			...
			if (!typeMatch) {
				var msg = 'Schema error: Impossible to extract type from metadata attribute ' + parts[0];
				log(msg);
				throw new Error(msg);
			}
...
module.exports.locatedError = function (originalError, nodes, path) {
...
module.exports.syntaxError = function (source, position, description) {
...
var blockStringValue = __webpack_require__(53);
 ...
 * whenever called.
 */
module.exports.createLexer = function (source, options) {
...
module.exports.TokenKind = {
   ...
   */
};module.exports.getTokenDesc = function (token) {
...
function printCharCode(code) {
  return (
    ...
    code < 0x007f ? (0, _stringify2.default)(String.fromCharCode(code)) : // Otherwise print the escaped form.
...
var _source = __webpack_require__(61);

var _error = __webpack_require__(32);

var _lexer = __webpack_require__(59);

var _kinds = __webpack_require__(58);

var _directiveLocation = __webpack_require__(54);
